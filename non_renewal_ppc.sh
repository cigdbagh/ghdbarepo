...
LOG_FILE="${LOG_PATH}"/"NON_RENEWAL_PPC_${SCHEMA}_${DBNAME}.lst"
TXT_FILE="${LOG_PATH}"/"NON_RENEWAL_PPC_${SCHEMA}_${DBNAME}.txt"
RM_LOG="${LOG_PATH}"/"RM_NON_RENEWAL_PPC_${S_HOST}.log"
...
MAIL_LOG_TO_DBAGRP="agrover@ciginsurance.com, SRajagopal@ciginsurance.com ,rkalyandrug@ciginsurance.com"
MAIL_LOG_TO_USER="${MAIL_LOG_TO_DBA} agrover@ciginsurance.com, SRajagopal@ciginsurance.com"

MAIL_LOG_SUB1="Daily Data Fixes (${U_S_HOST}): ERROR on PKG_PRINT_CALLIGO_BILLS in ${U_SCHEMA} of ${U_DBNAME}"
MAIL_LOG_SUB2="Daily Data Fixes (${U_S_HOST}): PKG_PRINT_CALLIGO_BILLS completed in ${U_SCHEMA} of ${U_DBNAME}"
RM_ERR="Daily Scripts (${U_S_HOST}): ERROR in removing PKG_PRINT_CALLIGO_BILLS logs."
...
${SCHEMA}/${PWD}@${DBNAME}
spool ${LOG_FILE}
--prompt Starting PKG_PRINT_CALLIGO_BILLS... 
prompt Starting SUBSCRIPTION RENEWAL LETTER FOR PPC... 
...
execute PKG_PRINT_CALLIGO_BILLS.sp_print_ppc_letter (null);
set time off timing off;
SELECT TO_CHAR(sysdate,'MM/DD/YY HH24:MI:SS') END_DATE from dual;
prompt SUBSCRIPTION RENEWAL LETTER FOR PPC complete.
spool off;
END_SQL
