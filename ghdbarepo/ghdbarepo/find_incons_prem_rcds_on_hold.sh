#!/bin/sh
# Name     :  find_incons_prem_rcds_on_hold.sh
# Author   :  Raman Paul
# created  :  04/16/12
...
SCR_FILE="${SCR_PATH}"/"find_incons_prem_on_hold.sql"
LOG_FILE="${LOG_PATH}"/"find_incons_prem_on_hold_${SCHEMA}_${DBNAME}.lst"
ERR_FILE="${LOG_PATH}"/"find_incons_prem_on_hold_${SCHEMA}_${DBNAME}.err"
TXT_FILE="${LOG_PATH}"/"find_incons_prem_on_hold_${SCHEMA}_${DBNAME}.txt"
RM_LOG="${LOG_PATH}"/"rm_find_incons_prem_on_hold_${S_HOST}.log"

##8##MAIL_LOG_TO_DBA="rpaul@ciginsurance.com"
...
MAIL_LOG_SUB1="Daily Scripts (${U_S_HOST}): ERROR on find_incons_prem_on_hold.sql in ${U_SCHEMA} of ${U_DBNAME}"
MAIL_LOG_SUB2="Daily Scripts (${U_S_HOST}): find_incons_prem_on_hold completed in ${U_SCHEMA} of ${U_DBNAME}"
MAIL_LOG_SUB3="Daily Scripts (${U_S_HOST}): Inconsistent PREM records with HOLD status found in ${U_SCHEMA} of ${U_DBNAME}"
RM_ERR="Daily Scripts (${U_S_HOST}): ERROR in removing find_incons_prem_on_hold logs."
...
${SCHEMA}/${PWD}@${DBNAME}
spool ${LOG_FILE}
prompt Starting FIND INCONSISTENT RECORDS IN PREM TABLE IN HOLD STATUS IN CIGADMIN... 
...
prompt FIND INCONSISTENT RECORDS IN PREM TABLE IN HOLD STATUS IN CIGADMIN complete.
spool off;
END_SQL
