...
#          :  02/07/05 - Sasi Sivadasan - Scripts are rewrote to include scripting standards.
...
SCR_FILE="${SCR_PATH}"/"fix_new_agency.sql"
LOG_FILE="${LOG_PATH}"/"fix_new_agency_${SCHEMA}_${DBNAME}.lst"
ERR_FILE="${LOG_PATH}"/"fix_new_agency_${SCHEMA}_${DBNAME}.err"
TXT_FILE="${LOG_PATH}"/"fix_new_agency_${SCHEMA}_${DBNAME}.txt"
RM_LOG="${LOG_PATH}"/"rm_fix_new_agency_${S_HOST}.log"
...
MAIL_LOG_SUB1="eDBA - Daily Data Fixes (${U_S_HOST}): ERROR on fix_new_agency.sql in ${U_SCHEMA} of ${U_DBNAME}"
MAIL_LOG_SUB2="Daily Data Fixes (${U_S_HOST}): Fix_new_agency.sql completed in ${U_SCHEMA} of ${U_DBNAME}"
RM_ERR="eDBA - Daily Data Fixes (${U_S_HOST}): ERROR in removing fix_new_agency logs."
...
${SCHEMA}/${PWD}@${DBNAME}
spool ${LOG_FILE}
prompt Starting FIX_NEW_AGENCY... 
...
prompt FIX_NEW_AGENCY complete.
spool off;
END_SQL
...
##########################################
# This procedure should be cancelled effective v5.1 CAPCOM
##########################################
