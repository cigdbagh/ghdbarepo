#!/bin/sh
# Name     :  rman_incremental_merge_prod.sh 
# Author   :  Raman Paul
# Created  :  05/12/08
...
#Modified  : Ramesh 08/13/15
#Comments  : Impleted  DD backup for Production
############################################################
#            MAIN                
############################################################
#################################
...
#################################
ORACLE_SID="cigprod1";export ORACLE_SID
...
##MAIL_LOG_SUB1="Rman Incr Merge Backup : ${U_DBNAME} RMAN Backup Failed  at ${END_JOB}"
##MAIL_LOG_SUB2="Rman Incr Merge Backup : ${U_DBNAME} RMAN Backup Succeeded at ${END_JOB}"
...
##      list copy tag weekly;
      ...
      ####delete force noprompt archivelog all backed up 2 times to sbt_tape;
      ####delete noprompt obsolete;
      run {  backup incremental level 1  for recover of  tag weekly  database ; }
      run {
      backup archivelog all ;
      backup current controlfile; 
        backup spfile;
