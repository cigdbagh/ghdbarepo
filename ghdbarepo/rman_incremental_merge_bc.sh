#!/bin/sh
# Name     :  rman_incremental_merge_bc.sh 
...
#################################
ORACLE_SID="bc1";export ORACLE_SID
...
##      list copy tag weekly;
      ...
      run {backup  incremental level 0 tag=weekly pluggable database bcprod,cmprod ; }
...
rman <<EOF > ${LOG_PATH}/rman_incr_merge_${DBNAME}_${DAY}_${TODAY}.log
      ...
      delete force noprompt archivelog all backed up 2 times to sbt_tape;
      delete noprompt obsolete;
      run {  backup incremental level 1 cumulative for recover  of tag weekly  pluggable database bcprod,cmprod ; }
      run {
