...
SCR_FILE="${SCR_PATH}"/"find_incons_prem_rcds.sql"
LOG_FILE="${LOG_PATH}"/"find_incons_prem_rcds1_${SCHEMA}_${DBNAME}.lst"
ERR_FILE="${LOG_PATH}"/"find_incons_prem_rcd1s_${SCHEMA}_${DBNAME}.err"
TXT_FILE="${LOG_PATH}"/"find_incons_prem_rcds1_${SCHEMA}_${DBNAME}.txt"
RM_LOG="${LOG_PATH}"/"rm_find_incons_prem_rcds1_${S_HOST}.log"
...
MAIL_LOG_TO_USER="${MAIL_LOG_TO_DBA} kkleven@ciginsurance.com dchaudhary@ciginsurance.com rbijalwan@ciginsurance.com"

MAIL_LOG_SUB1="Daily Scripts (${U_S_HOST}): ERROR on find_incons_prem_rcds.sql in ${U_SCHEMA} of ${U_DBNAME}"
MAIL_LOG_SUB2="Daily Scripts (${U_S_HOST}): find_incons_prem_rcds.sql completed in ${U_SCHEMA} of ${U_DBNAME}"
MAIL_LOG_SUB3="Daily Scripts (${U_S_HOST}): Inconsistent PREM records found in ${U_SCHEMA} of ${U_DBNAME}"
RM_ERR="Daily Scripts (${U_S_HOST}): ERROR in removing find_incons_prem_rcds logs."
...
${SCHEMA}/${PWD}@${DBNAME}
spool ${LOG_FILE}
prompt Starting FIND INCONSISTENT RECORDS IN PREM TABLE IN CIGADMIN... 
...
prompt FIND INCONSISTENT RECORDS IN PREM TABLE IN CIGADMIN complete.
spool off;
END_SQL
