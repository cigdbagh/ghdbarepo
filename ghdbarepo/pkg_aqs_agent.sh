#!/bin/sh
# Name     :  pkg_aqs_agent.sh
# Author   :  Shashi Pavan
# Created  :  01/07/98
# Desc     :  This particular shell script is used to execute pkg_aqs_agent.sp_aqs_agent in CIGADMIN schema.
...
LOG_FILE="${LOG_PATH}"/"pkg_aqs_agt_sp_aqs_agt_${SCHEMA}_${DBNAME}.lst"
TXT_FILE="${LOG_PATH}"/"pkg_aqs_agt_sp_aqs_agt_${SCHEMA}_${DBNAME}.txt"
RM_LOG="${LOG_PATH}"/"rm_pkg_aqs_agt_sp_aqs_agt_${S_HOST}.log"
...
MAIL_LOG_SUB1="eDBA - Weekly Scripts (${U_S_HOST}): ERROR on pkg_aqs_agent.sp_aqs_agent in ${U_SCHEMA} of ${U_DBNAME}"
MAIL_LOG_SUB2="Weekly Scripts (${U_S_HOST}): Pkg_aqs_agent.sp_aqs_agent completed in ${U_SCHEMA} of ${U_DBNAME}"
RM_ERR="eDBA - Weekly Scripts (${U_S_HOST}): ERROR in removing pkg_aqs_agent.sp_aqs_agent logs."
...
${SCHEMA}/${PWD}@${DBNAME}
spool ${LOG_FILE}
prompt Starting PKG_AQS_AGENT.SP_AQS_AGENT... 
...
execute pkg_aqs_agent.sp_aqs_agent;
set time off timing off;
SELECT TO_CHAR(sysdate,'MM/DD/YY HH24:MI:SS') END_DATE from dual;
prompt PKG_AQS_AGENT.SP_AQS_AGENT complete.
spool off;
END_SQL
