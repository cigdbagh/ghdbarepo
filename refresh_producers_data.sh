#!/bin/sh
# Name     :  refresh_producers_data.sh
# Author   :  Jaspal
# Created  :  12/10/15 
...
LOG_FILE="${LOG_PATH}"/"REFRESH_PRODUCERS_DATA_${SCHEMA}_${DBNAME}.lst"
TXT_FILE="${LOG_PATH}"/"REFRESH_PRODUCERS_DATA_${SCHEMA}_${DBNAME}.txt"
RM_LOG="${LOG_PATH}"/"RM_REFRESH_PRODUCERS_DATA_${S_HOST}.log"
...
#MAIL_LOG_TO_DBAGRP='shdutt@ciginsurance.com,mgraziano@ciginsurance.com,jsingh@ciginsurance.com'
MAIL_LOG_TO_USER=`sh ${PWD_PATH}/mail_log_to_dba.sh`

MAIL_LOG_SUB1="Daily Data Fixes (${U_S_HOST}): ERROR on REFRESH_PRODUCERS_DATA in ${U_DBNAME}"
MAIL_LOG_SUB2="Daily Data Fixes (${U_S_HOST}): REFRESH_PRODUCERS_DATA completed in ${U_SCHEMA} of ${U_DBNAME}"
RM_ERR="Daily Scripts (${U_S_HOST}): ERROR in removing REFRESH_PRODUCERS_DATA logs."
...
${SCHEMA}/${PWD}@${DBNAME}
spool ${LOG_FILE}
prompt Starting REFRESH PRODUCERS DATA ... 
...
@${SCR_PATH}/RefreshProducersData.sql
set time off timing off;
SELECT TO_CHAR(sysdate,'MM/DD/YY HH24:MI:SS') END_DATE from dual;
prompt REFRESH_PRODUCERS_DATA complete.
spool off;
END_SQL
